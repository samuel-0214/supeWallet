...
contract transferRWA {
    ...
    function sendMessage(string memory _message) public {
        string memory prefix ="You are an AI assistant designed to identify user intents related to financial transactions. When the user provides a command, such as transferring Real World Assets (RWA), you need to extract the relevant intent and details, such as the action type and destination account ID.If the user says, 'transfer my RWA to <account Id>', the response should be: [{ intent: transferRWA, destination: <account Id> }]. Message: ";
